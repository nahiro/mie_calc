      SUBROUTINE TMATRIX(IFLG,IERR,RAT,NDISTR,AXMAX,NPNAX,R1,R2,B,GAM,
     &                   NKMAX,DEPS,NP,DLAM,DMRR,DMRI,DDELT,NPNA,NDGS,
     &                   REFF_O,VEFF_O,CEXT_O,CSCA_O,WALB_O,ASYM_O,
     &                   LMAX_O,
     &                   ALP1_O,ALP2_O,ALP3_O,ALP4_O,BET1_O,BET2_O,
     &                   FMAT_O)
...
C   ANY DAMAGES THAT MAY RESULT FROM THE USE OF THE PROGRAM.
     ...
     *        DDR(NPNG2),DRR(NPNG2),DRI(NPNG2),ANN(NPN1,NPN1)
     ...
     &        AL1(NPL),AL2(NPL),AL3(NPL),AL4(NPL),BE1(NPL),BE2(NPL)
      INTEGER LMAX_O(NAXMAX)
      REAL*8  REFF_O(NAXMAX),VEFF_O(NAXMAX),
     &        CEXT_O(NAXMAX),CSCA_O(NAXMAX),
     &        WALB_O(NAXMAX),ASYM_O(NAXMAX),
     &        ALP1_O(NALMAX),ALP2_O(NALMAX),
     &        ALP3_O(NALMAX),ALP4_O(NALMAX),
     &        BET1_O(NALMAX),BET2_O(NALMAX),
     &        FMAT_O(NARMAX)
      REAL*8  FFA(NAFMAX)
      REAL*4
...
C  OPEN FILES *******************************************************
      ...
      IF(IFLAG.GT.0) THEN
        INQUIRE(FILE='tmx_out3.dat',OPENED=OPN)
        IF(.NOT.OPN) OPEN( 6,FILE='tmx_out3.dat')
        INQUIRE(FILE='tmx_out4.dat',OPENED=OPN)
        IF(.NOT.OPN) OPEN(10,FILE='tmx_out4.dat')
      ENDIF
...
C  INPUT DATA ********************************************************
      ...
      IF(IFLAG.GT.0) WRITE (6,5454) NCHECK
...
C        R2=1.56538D0*AXI
 ...
 8001    FORMAT ('NK=',I4,' I.E., IS GREATER THAN 1000. ',
    ...
    4    CALL DISTRB (NK,XG1,WG1,NDISTR,AXI,B,GAM,R1,R2,
     &                REFF,VEFF,PIN)
         IF(IFLAG.GT.0) PRINT 8002,R1,R2
     ...
     &       '.  EXECUTION TERMINATED')
            ...
            DO 50 NMA=INM1,NPN1
               ...
               IF (NGAUSS.GT.NPNG1) THEN
                 IF(IFLAG.GT.0) PRINT 7340, NGAUSS
                 IERR = 3
                 RETURN
               ENDIF
...
c              PRINT 7337, NGGG,DSCA,DEXT
  ...
  204       CONTINUE
            IF (NMAX1.GT.NPN4) THEN
              IF(IFLAG.GT.0) PRINT 7550, NMAX1
              IERR = 5
              RETURN
            ENDIF
...
C           PRINT 6070, I,NMAX,NMAX1,NGAUSS
 6070       FORMAT(4I6)
  ...
  510    CONTINUE
         ...
         ASYM_O(IAX) = ASYMM
         LMAX_O(IAX) = L1MAX
         IF(IFLAG.GT.0) PRINT 9100,CEXTIN,CSCAT,WALB,ASYMM
 9100    FORMAT('CEXT=',D12.6,2X,'CSCA=',D12.6,2X,
     &          2X,'W=',D12.6,2X,'<COS>=',D12.6)
         IF (WALB.GT.1D0) THEN
           IF(IFLAG.GT.0) PRINT 9111
         ENDIF
 9111    FORMAT ('WARNING: W IS GREATER THAN 1')
         IF(IFLAG.GT.0) WRITE (10,580) WALB,L1MAX
         DO L=1,L1MAX
            K = (IAX-1)*NPL+L
            ALP1_O(K) = ALPH1(L)
            ALP2_O(K) = ALPH2(L)
            ...
            BET1_O(K) = BET1(L)
            BET2_O(K) = BET2(L)
            IF(IFLAG.GT.0) WRITE (10,575)
     &                     ALPH1(L),ALPH2(L),ALPH3(L),ALPH4(L),
     &                     BET1(L),BET2(L)
         ENDDO
  575    FORMAT(6D14.7)
  580    FORMAT(D14.8,I8)
         LMAX=L1MAX-1
         CALL MATR (ALPH1,ALPH2,ALPH3,ALPH4,BET1,BET2,LMAX,NPNA,FFA)
         DO I1=1,NPNA
           K = (I1-1)*6
           L = (IAX-1)*NAFMAX+K
           FMAT_O(L+1) = FFA(K+1)
...
C**********************************************************************
     ...
     *        AN(NPN1),ANN(NPN1,NPN1),DD(NPN1)
      ...
      DO 10 N=1,NMAX
           NN=N*(N+1)
           AN(N)=REAL(NN,16)
           D=SQRTQ(REAL(2*N+1,16)/REAL(NN,16))
           DD(N)=D
           DO 10 N1=1,N
                DDD=D*DD(N1)*0.5Q0
                ANN(N,N1)=DDD
...
c         PRINT 5000, CHECK
   ...
   20         PC=X*PB+(X*PB-PA)*(DJ-A)/DJ
          ...
          PB=PA*PC*(A-X*X)
          X=X-PB
          IF(FABSQ(PB).GT.CHECK*FABSQ(X)) GO TO 10
          Z(M)=X
          W(M)=PA*PA*(A-X*X)
 ...
 5000 format ('QGAUSS DOES NOT CONVERGE, CHECK=',E10.3)
      ...
      IF(IFLAG.GT.0) PRINT 1100,N
 1100 FORMAT(' ***  POINTS AND WEIGHTS OF GAUSSIAN QUADRATURE FORMULA',
     * ' OF ',I4,'-TH ORDER')
      DO 105 I=1,K
          ZZ=-Z(I)
  105     IF(IFLAG.GT.0) PRINT 1200,I,ZZ,I,W(I)
...
C**********************************************************************
   ...
   10 CONTINUE
      IF (NMAX.GT.NPN1) THEN
        IF(IFLAG.GT.0) PRINT 9000,NMAX,NPN1
        IERROR = 6
        RETURN
      ENDIF
 9000 FORMAT(' NMAX = ',I2,', i.e., greater than ',I3)
      TB=TA*SQRTQ(MRR*MRR+MRI*MRI)
      TB=FMAXQ(TB,REAL(NMAX,16))
      NNMAX1=8.0Q0*SQRTQ(FMAXQ(TA,REAL(NMAX,16)))+3Q0
      NNMAX2=(TB+4Q0*(TB**0.33333Q0)+8.0Q0*SQRTQ(TB))
...
C**********************************************************************
     ...
     *              (DN-1Q0)-0.25Q0*EP*EPS/(9Q0*DN-1Q0)
      ...
      DO 50 I=1,NG
         XI=ACOSQ(X(I))*DNP
         RI=R0*(1Q0+EPS*COSQ(XI))
         R(I)=RI*RI
         DR(I)=-R0*EPS*DNP*SINQ(XI)/RI
   50 CONTINUE
      RETURN
      END
...
C**********************************************************************
      ...
      L=NMAX+NNMAX
      XX=1Q0/X
      Z(L)=1Q0/(REAL(2*L+1,16)*XX)
      L1=L-1
      DO 5 I=1,L1
         I1=L-I
         Z(I1)=1Q0/(REAL(2*I1+1,16)*XX-Z(I1+1))
    5 CONTINUE
      Z0=1Q0/(XX-Z(1))
      Y0=Z0*COSQ(X)*XX
      Y1=Y0*Z(1)
...
C**********************************************************************
      ...
      NMAX1=NMAX-1
      DO 5 I=2,NMAX1
    5     Y(I+1)=REAL(2*I+1,16)*X1*Y(I)-Y(I-1)
      V(1)=-X1*(C+Y1)
      DO 10 I=2,NMAX
  10       V(I)=Y(I-1)-REAL(I,16)*X1*Y(I)
      RETURN
      END
...
c     0.LE.x.LE.1
   ...
   20 QMM=REAL(M*M,16)
      DO 25 I=1,M
         I2=I*2
         A=A*SQRTQ(REAL(I2-1,16)/REAL(I2,16))*QS
   25 CONTINUE
      D1=0Q0
      D2=A
      DO 30 N=M,NMAX
...
C	Inversion from LAPACK
      ...
      DO I=1,NNMAX
        DO J=1,NNMAX
          ZQ(I,J)=QR(I,J)+QI(I,J)*(0.0q0,1.0q0)
        ENDDO
      ENDDO
      INFO=0
...
C****************************************************************
     ...
     &                   VEFF,PI)
      ...
      IF(IFLAG.GT.0) PRINT 1001,AA,BB,GAM
 1001 FORMAT('MODIFIED GAMMA DISTRIBUTION, alpha=',F6.4,'  r_c=',
   ...
   50 CONTINUE
      GO TO 400
  100 IF(IFLAG.GT.0) PRINT 1002,AA,BB
 ...
 1003 FORMAT('POWER LAW DISTRIBUTION OF HANSEN & TRAVIS 1974')
  ...
  250 CONTINUE
      GO TO 400
  300 IF(IFLAG.GT.0) PRINT 1004,AA,BB
 1004 FORMAT ('GAMMA DISTRIBUTION,  a=',F8.4,'  b=',F6.4)
  ...
  350 CONTINUE
      GO TO 400
  360 IF(IFLAG.GT.0) PRINT 1005,BB
...
C*************************************************************
      ...
      DO 100 L=1,L1
         ...
         DO 50 I=1,11
            ...
            IF((DL-C2)*(DL+C3)-C1.LE.-1D-4) KONTR=2
            IF(KONTR.EQ.2) THEN
              IF(IFLAG.GT.0) PRINT 4000,LL,C
            ENDIF
   50    CONTINUE
  100 CONTINUE
      IF(KONTR.EQ.1) THEN
        IF(IFLAG.GT.0) PRINT 2000
      ENDIF
...
C        180*(I-1)/(N-1) (DEGREES), WHERE I NUMBERS THE ANGLES
      ...
      SUBROUTINE MATR(A1,A2,A3,A4,B1,B2,LMAX,NPNA,FFA)
      INCLUDE 'tmq.par.f'
      IMPLICIT REAL*8 (A-H,O-Z)
      ...
      DB=180D0*DN
      L1MAX=LMAX+1
      IF(IFLAG.GT.0) PRINT 1000
 1000 FORMAT(' ')
      IF(IFLAG.GT.0) PRINT 1001
 1001 FORMAT(' ',2X,'S',6X,'ALPHA1',6X,'ALPHA2',6X,'ALPHA3',
     &       6X,'ALPHA4',7X,'BETA1',7X,'BETA2')
      DO 10 L1=1,L1MAX
         L=L1-1
         IF(IFLAG.GT.0) PRINT 1002,
     &                  L,A1(L1),A2(L1),A3(L1),A4(L1),B1(L1),B2(L1)
   10 CONTINUE
 1002 FORMAT(' ',I3,6F12.5)
      TB=-DB
      TAA=-DA
      IF(IFLAG.GT.0) PRINT 1000
      IF(IFLAG.GT.0) PRINT 1003
...
C        F34=F34/F11
         ...
         FFA(K+5) = F12
         FFA(K+6) = F34
         IF(IFLAG.GT.0) PRINT 1004,TB,F11,F22,F33,F44,F12,F34
  500 CONTINUE
      IF(IFLAG.GT.0) PRINT 1000
